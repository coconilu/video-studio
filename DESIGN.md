...
| D12 | 桌面壳落地（2026-09-02） | D2 二期提前：新增自包含 `desktop/`（Tauri v2，不进任何 workspace）。壳极薄——spawn Node sidecar 跑 `server/index.mjs`，读 stdout 的 `video-studio listening:` 就绪行，启动页轮询 `api_origin` 命令后跳转；**壳内无业务前端**，复用 server 托管的 `web/`。开发模式用系统 Node + 仓库代码（`CARGO_MANIFEST_DIR` 上两级探测）；安装包由 `desktop/scripts/bundle-*.mjs` 内嵌 Node 独立运行时 + 平台代码副本到 `src-tauri/resources/`。端口：4173 空闲则用，否则随机端口经 `STUDIO_PORT` 传入。Windows sidecar 与退出清理沿用静默纪律（`CREATE_NO_WINDOW` + `taskkill /T /F`）。同期落地**环境自检**：`tools/doctor.mjs` 逐项探测五项外部依赖（Node/kimi/ffprobe/chrome-headless-shell/media-hub，chrome 会扫 Puppeteer 缓存里任意 150 版本而非死磕单一路径），`GET /api/doctor` 暴露，web 启动闸门有红灯弹面板（MOCK 不挡）、设置页可重跑。自动更新同日落地（方案 B）：`tauri-plugin-updater` + 新签 minisign 密钥对（公钥写死 `desktop/src-tauri/tauri.conf.json`，私钥与密码只存仓库 Secrets）；更新提示在**启动页**（app origin，可用 IPC）跳转前检查一次，失败静默——平台 UI 是 http origin 拿不到 IPC，30 分钟轮询因此不做。`createUpdaterArtifacts` 仅在 release workflow 经 `--config` 开启。发布自动化：`.github/workflows/release.yml` 三段式（bump PR → win/mac matrix → latest.json + Release，issue 关闭触发打 prerelease），`.github/workflows/ci.yml` 为 PR 门禁（smoke + cargo check）。已知坑：Git Bash 把 NSIS `/S` 转成路径（需 `MSYS_NO_PATHCONV=1`）；Tauri resources 的 glob 不匹配子目录（须显式列 `resources/node/` 等）。macOS 桌面版首期未实测（dmg 在构建矩阵里，但 GUI PATH 极简会影响 kimi/ffprobe 探测）。 |
...
## 12. Agent 接入（D11，2026-09-01）

- **API 即接口**：`server/index.mjs` 路由表就是 agent 的完整操作面（建任务 / 轮询 / approve / reject / gates 覆盖 / fork / 制品读写 / 日志 / 历史）。`POST /api/tasks` 的 body 额外接受 `gates`：对象 `{stageId: gate}` 逐阶段覆盖（建目录前校验，非法即 400），或字符串 `"auto"` 展开为全阶段 auto。
- **候选阶段的 auto 语义**：`executeStage` 完成时若 `candidates>1` 且闸门非 `required`，自动 `copyChoice(…, 1)` 并记录 `chosen:1`。
- **skill 分发**：`skill/SKILL.md` 是 agent 驱动说明书（frontmatter 带 `version`）。`core/skill.mjs` 提供 status/install/uninstall，路由为 `GET /api/skill`、`POST /api/skill/install`、`POST /api/skill/uninstall`；install 先清空目标目录再整体复制（不留残留文件），uninstall 只删 `<SKILLS_DIR>/video-studio/` 一层（路径安全校验）。更新检测 = 源与已装 SKILL.md 内容逐字节比对。设置页（顶栏「⚙ 设置」）是这三个接口的 UI。
